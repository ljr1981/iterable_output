...
feature -- Output
	...
	out_json: STRING
		do
			Result := out_json_internal
			Result.remove_tail (2)
			Result.append_character ('}')
			Result.append_character ('%N')
		end

feature {READABLE_INDEXABLE_EXT} -- Implementation

	out_json_internal: STRING
			--
		do
			create Result.make_empty
			if attached {READABLE_INDEXABLE [ANY]} Current as al_current then
				...
				loop
					...
					if attached {READABLE_INDEXABLE_EXT} ic.item as al_row then
						Result.append_string_general (al_row.out_json_internal)
						-- Plain ole READABLE_INDEXABLE things might possibly be converted and output.
					elseif attached {READABLE_INDEXABLE [ANY]} ic.item as al_row then
						if attached {ARRAY2 [ANY]} al_row as al_item then
							Result.append_string_general (array2_ext_from_array2 (al_item).out_json_internal)
						elseif attached {ARRAYED_LIST [ANY]} al_row as al_item then
							Result.append_string_general (arrayed_list_ext_from_arrayed_list (al_item).out_json_internal)
						elseif attached {ARRAYED_STACK [ANY]} al_row as al_item then
							Result.append_string_general (arrayed_stack_ext_from_arrayed_stack (al_item).out_json_internal)
						elseif attached {ARRAY [ANY]} al_row as al_item then
							Result.append_string_general (array_ext_from_array (al_item).out_json_internal)
						elseif attached {HASH_TABLE [ANY, detachable HASHABLE]} al_row as al_item then
							Result.append_string_general (hash_table_ext_from_hash_table (al_item).out_json_internal)
						elseif attached {STRING_TABLE [ANY]} al_row as al_item then
							Result.append_string_general (string_table_ext_from_string_table (al_item).out_json_internal)
						elseif is_basic_type (ic.item) and then attached ic.item as al_item then
							Result.append_character ('"')
							...
							Result.append_string_general ("n/a"); Result.append_character ('}'); Result.append_character (',')
							check unknown_condition: False end
						end
					elseif is_basic_type (ic.item) and then attached ic.item as al_item then
						Result.append_string_general (al_item.out); Result.append_character ('}'); Result.append_character (',')
					else -- Otherwise, we don't want complex outputs.
						Result.append_string_general ("n/a"); Result.append_character (',')
					end
					if attached {HASH_TABLE [ANY, detachable HASHABLE]} al_current as al_hash_table then
